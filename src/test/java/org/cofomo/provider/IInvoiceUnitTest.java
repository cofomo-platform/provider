	...
	public void shouldReturnOneInvoice() throws Exception {
		...
		this.mockMvc.perform(get("/v1/invoice/1").accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
				.andDo(document("invoice-get-by-id", preprocessRequest(prettyPrint()), preprocessResponse(prettyPrint()),
						responseFields(fieldWithPath("id").description("Autogenerated UUID of the booking").type("String"),
								fieldWithPath("paymentProviderId").description("Id of payment provider").type("String"),
								fieldWithPath("clientId").description("Id of corresponding client").type("String"),
								fieldWithPath("status").description("Status of invoice").type("InvoiceStatus"),
								fieldWithPath("lastUpdate").description("Last update of invoice").type("DateTime"),
								fieldWithPath("value").description("Value of invoice").type("Number"),
								fieldWithPath("currency").description("Currency of invoice").type("String"))))
				.andDo(print());
	}
